...
export class ProductRepository implements IProductRepository {
  ...
  async findAll(limit: number, offset: number): Promise<Product[]> {
    // Paginação simples por limit/skip para listagens.
  ...
  async addCategory(productId: string, categoryId: string): Promise<void> {
    await ProductSchema.findOneAndUpdate(
      ...
      {
        // Mantem apenas a referencia da categoria no produto.
        $push: {
          categories: {
            _id: categoryId,
          },
  ...
  async removeCategory(productId: string, categoryId: string): Promise<void> {
    await ProductSchema.findOneAndUpdate(
      ...
      {
        // Remove a referencia da categoria sem excluir a categoria em si.
        $pull: {
          categories: {
            _id: categoryId,
          },
